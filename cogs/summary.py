...
class Summary(commands.Cog):
    ...
    async def collect(self, ctx, num):
        ...
        await ctx.channel.send(
            f"Collected the last {num_messages} messages and saved them to {file_name}"
        )
        ...
        all_chat_path = os.path.join(
            vector_store_directory + f"/{ctx.guild.id}", file_name
        )
        ...
        if os.path.exists(file_directory):
            ...
            if str(response.status_code) == "201":
                summary_embed.add_field(
                    name=f"Summary generated in {(generation_end-generation_start):.2f} secs saved to server in {(end_time-generation_end):.2f} secs",
                    value=f"your summary has been saved successfully",
                )
                ...
                login_embed_url = f"{login_url}"
                summary_embed.add_field(
                    name=f"Login to save summaries (generated in ({(generation_end-generation_start):.2f} secs)",
                    value=f"[Login Here]({login_embed_url})",
                )
